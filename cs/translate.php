<?php

/*
Section: translate
Language: Czech
Translator: SierraHawk <sierrahawk@outlook.com>
*/

$translations = [
    'h1' => 'Překlady',

    'l10ncredits' => 'Mnohokrát děkujeme všem překladatelům',

    'l10ncreditsclick' => 'Klikněte pro zobrazení seznamu',

    'p1a' => 'Po aktualizaci stránek v roce 2014 db4free.net a služeb je nyní snadnější způsob, jak můžete přeložit webové stránky db4free.net do více jazyků.',

    'p1b' => 'Všechny texty naleznete na stránkách <a href="https://github.com/">GitHubu</a>. Můžete si stáhnout repozitář, a použít pro překlad. Zašlete nám žádost o pull, takže vaše překlady mohou být integrovány do webové stránky.',

    'h2' => 'Jak to funguje?',

    'p2a' => 'Nejprve se musíte zaregistrovat účet na GitHubu, pokud již máte účet, tak stačí kliknout na následující odkaz pro stažení:',

    'p2b' => '<a href="https://github.com/mpopp75/db4free-net-l10n">https://github.com/mpopp75/db4free-net-l10n</a>',

    'p2c' => ' takže budete mít identickou kopii překladu na Vašem účtu. Nyní si můžete naklonovat repozitář a vytvořit kopii ve vašem systému. Příkaz git pro to je:',

    'p2d' => 'git clone git@github.com:[vase_github_jmeno]/db4free-net-l10n.git',

    'p2e' => 'Doporučujeme Vám vytvořit samostatnou větev pro vaše překlady:',

    'p2f' => 'git checkout -b mytranslations',

    'p2g' => 'Není-li uvden adresář pro Váš jazyk, který chcete použít pro překlady, tak vytvořte kopii adresáře "en" a názevěte jej podle <a href = "http://en.wikipedia.org/wiki/List_of_ISO_639-1_codes "> ISO 639-1 kódů jazyků </a>.',

    'p2h' => 'V souborech uvnitř tohoto adresáře, nahradite anglické texty novým překladem. Prosím, buďte opatrní, abyste nezměnily ID. Příklad nahrazení:',

    'p2i' => "'h1' => 'Welcome on db4free.net',",

    'p2j' => 'na',

    'p2k' => '\'h1\' => \'Vítejte na db4free.net\',',

    'p2l' => 'Je-li text stejný v angličtině i v jazyku do kterého překládáte, můžete odstranit řádek s textem nebo jej ponechat.',

    'p2m' => 'Pokud jste soubor přeložily, pak už jen stačí nahrát tyto soubory na úložiště GitHubu. Přepnout na novou pobočku na GitHubu a kliknout na tlačítko "Nový Pull Požadavek". Zadejte krátký popisek a je hotovo.',

    'p2n' => 'Chcete-li přispívat, ale budete potřebovat další pomoc, než budete moci začít, prosím <a href="https://github.com/mpopp75/db4free-net-l10n/issues">otevřete nový požadavek na GitHubu</a>.',
];
