<?php
/*
Section: أهلا بك 
Language: العربية
*/

$translations = array(
    'h1' => 'أهلا بك في  db4free.net',

    'p1a' => 'db4free.net تقدم لك خدمة اختبار بأحدث  - وأحيانا حتى التطوير - نسخ من <a href="https://www.mysql.com/">MySQL Server</a>. يمكنك بسهولة <a href="/signup.php">إنشاء حساب مجاناً</a> 
    واختبار تطبيقاتك ، على سبيل المثال للتأكد من ان التطبيق لازال يعمل بعد تحديث نسخة MySQL.
 db4free.net هو مصدر جيد للتعلم و لدمجك مع الميزات الجديدة التي تقدمها النسخ الجديدة.',

    'p1b' => 'db4free.net  بأحدث إصدار قيد التطوير  تهدف إلى تزويدك إما بأحدث إصدار أو. db4free.net\'s MySQL server سيتم تحديثه قريبا بعد عدة إصدارات ، بنفس الموعد أو أيام قليلة تليه.',

    'p1c' => 'للوصول إلى بياناتك بطريقة موثوقة, db4free.net يقدم لك أحدث نسخة من <a href="https://www.phpmyadmin.net">phpMyAdmin</a>. phpMyAdmin سيتم تحديثها بانتظام بحيث يتم الحصول دائما على أحدث النسخ.',

    'h2' => 'db4free.net ما هي الأمور التي لا يقدمها ',

    'p2' => 'db4free.net هو <b>خدمة اختبار</b>   وهذا يعني أنه غير مناسب للتطوير . يمكن أن يكون هناك انهيارات ، فقدان للبيانات أو مشاكل بالحماية لا تتوافق مع المعايير التي تقدمها مخدمات استضافة البيانات الاحترافية. إذا كنت بحاجة لقواعد بيانات MySQL  من أجل التطوير ، فضلاً لا تستخدم db4free.net!',

    'h3' => 'هل يمكنك المساعدة في ترجمة db4free.net?',

    'p3' => 'هل تتكلم لغة أخرى لا يدعمها موقعنا ؟ ، يمكنك مساعدتنا في ترجمة db4free.net. <a href="/translate.php">تعلم كيف !</a>',

    'h4' => 'المصادر',

    'p4a' => 'هذا  <a href="https://www.mpopp.net/category/db4free/">db4free.net قسم </a> في <a href="https://www.mpopp.net/">mpopp.net blog</a> أخبار ال  db4free.net. اشترك ب <a href="https://www.mpopp.net/category/db4free/feed/">RSS Feed</a> للتأكد من متابعتك لآخر المستجدات . db4free.net <a href="/twitter.php">Twitter</a>, مصدر آخر للاطلاع على آخر أخبار عالم db4free.net.',

    'p4b' => 'أفض مصادر لتعلم  MySQL a <a href="https://dev.mysql.com/">MySQL Developer Zone</a>, the <a href="https://dev.mysql.com/doc/refman/8.0/en/">MySQL الدليل اليدوي</a> and <a href="https://planet.mysql.com/">PlanetMySQL</a>. The MySQL website offers a number of <a href="https://dev.mysql.com/tech-resources/articles/">Developer Articles</a> والتي معظمها تشرح المزايا الخاصة بالنسخ الجديدة والقادمة بتفاصيل رائعة.',

    'p4c' => 'عند ظهور مشكلة في مخدم  MySQL,من فضلك قم بإضافة تقرير إلى  <a href="https://bugs.mysql.com/">MySQLنظام تتبع الأخطاء </a>.',
);
?>

