<?php
/*
Section: conditions
Language: Turkish, Türkçe
*/

$translations = array(
    'h1' => 'Kullanım Şartları',

    'p1' => 'db4free.net ekibi olarak herhangi bir durumda herhangi bir kişiye/kuruma;bu anasistemin kullanımından yada kendisinin,hizmetlerinin,yazılımının, erişim dışı kalışından kaynaklanan kazanç kaybı da dahil olmak üzere,doğrudan,dolaylı,spesifik,tesadüfi veya zamanla hasıl olan zararlardan dolayı yönetim bu zararlara karşı uyarılmış olsa da sorumluluk kabul etmeyeceğimizi bildirmek isteriz.',

    'p2' => 'Ekibimiz bu uygulamanın pazarlanabilirliğinin ve özel bir amaca uyumluluğunun sorumluluğunu özellikle reddetmekedir.Sağlanan yazılım ve host "as is" esaslıdır ve ekibimizin bakım,destek,güncelleme,geliştirme,düzenleme veya hizmetleri sürekli kullanılabilir yapma zorunluluğu bulunmamaktadır.Ekibimiz bu hizmeti önceden bildiri yapmadan durdurma ve verileri kalıcı olmak yetkisine sahiptir.',

    'p3' => 'db4free.net kullanıcıları kendileri yada sunucularla ilgili herhangi bir veriyi 3.kişilerle paylaşma hakkına sahip değildir.',

    'p4' => 'db4free.net projesi ile ilgili hizmetlere yalnız Avusturya Cumhuriyeti yasaları uygulanır.',

    'p5' => 'db4free.net veritabanını önemli bir veri yada amaç için kullanmanızı tavsiye etmiyoruz! Bizler birbirimize şirket üzerinden bağlantılı olsakda, bu özel ve ticari olmayan bir projedir.Herhangi bir garantiyi ve sorumluluğu reddediyoruz!',

    'p6' => 'db4free.net ekibi bağlantılanmış hiçbir sayfanın sorumluluğunu üstlenmemektedir.',

    'p7' => 'Bu sözleşmeyi önceden bildiri yapmadan değiştirme hakkı saklıdır.',

    'p8' => 'Heidenreichstein, Avusturya<br>29 Haziran 2014',
);
?>