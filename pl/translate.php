<?php
/*
Section: translate
Language: Polish
*/

$translations = array(
    'h1' => 'Tłumaczenia',

    'l10ncredits' => 'Wielkie dzięki dla osób mających wkład w tłumaczenia',

    'l10ncreditsclick' => 'Kliknij aby zobaczyć listę',

    'p1a' => 'Po aktualizacji strony oraz serwisu db4free.net w roku 2014 istnieje teraz prosty sposób tłumaczenia strony db4free.net na więcej języków.',

    'p1b' => 'Wszystkie teksty strony są dostępne na <a href="https://github.com/">GitHubie</a>. Możesz rozwidlić repozytorium (operacja fork), przetłumaczyć teksty a następnie wysłać rezultat jako git pull, aby Twoja praca została zintegrowana ze stroną.',

    'h2' => 'Jak to działa?',

    'p2a' => 'Jeśli nie posiadasz jeszcze konta na GitHubie, musisz je założyć. Dokonaj rozwidlenia repozytorium (fork) na ',

    'p2b' => '<a href="https://github.com/mpopp75/db4free-net-l10n">https://github.com/mpopp75/db4free-net-l10n</a>',

    'p2c' => ' tak abyś posiadł identyczną kopię repozytorium tłumaczeń na swoim koncie GitHub. Teraz możesz sklonować repozytorium aby uzyskać lokalną kopię na swoim komputerze. Polecenie git jakie musisz wykonać to:',

    'p2d' => 'git clone git@github.com:[Your_GitHub_username]/db4free-net-l10n.git',

    'p2e' => 'Zalecamy utworzenie osobnej gałęzi dla Twoich zmian, na przykład:',

    'p2f' => 'git checkout -b mytranslations',

    'p2g' => 'Jeśli nie widzisz katalogu dla języka, na jaki chcesz przetłumaczyć stronę, skopiuj katalog „en” i nazwij go zgodnie ze standardem <a href="http://en.wikipedia.org/wiki/List_of_ISO_639-1_codes">ISO 639-1</a>.',

    'p2h' => 'W tymże skopiowanym katalogu, zmień teksty angielskie na teksty nowego języka. Prosimy uważać, aby nie zmieniać identyfikatorów tekstów. Przykładowa zmiana może wyglądać tak:',

    'p2i' => '\'h1\' => \'Welcome to db4free.net\',',

    'p2j' => 'na',

    'p2k' => '\'h1\' => \'Witamy w db4free.net\',',

    'p2l' => 'Jeśli angielski pokrywa się przez przypadek z tekstem w Twoim języku, możesz usunąć linię z tekstem lub po prostu pozostawić ją bez zmian.',

    'p2m' => 'Kiedy tłumaczenie jest skończone, zatwierdź zmiany (git commit) oraz wykonaj operację push do swojego repozytorium GitHuba. Przełącz się na nowoutworzoną gałąź w GitHubie i kliknij „New Pull Request”. Wprowadź krótki tekst opisujący Twoje tłumaczenie, problemy, które napotkałeś lub które z tłumaczeń nie byłeś w stanie dokonać itd. itp. i wyślij Pull Request.',

    'p2n' => 'Jeśli chciałbyś mieć wkład w tłumaczenie lecz potrzebujesz pomocy zanim zaczniesz, prosimy, <a href="https://github.com/mpopp75/db4free-net-l10n/issues">utwórz nową wiadomość na GitHubie</a>.',
);
?>