<?php
/*
Section: signup
Language: Spanish Español
Translator: Cristiam Mercado <cristiammercadoj@gmail.com>
*/

$translations = array(
    'h1' => 'Registro',

    'mysql-db-name' => 'Nombre de la base de datos MySQL',

    'mysql-user-name' => 'Nombre de usuario MySQL',

    'mysql-user-password' => 'Contraseña de usuario MySQL',

    'mysql-user-password-verification' => 'Verifique contraseña de usuario MySQL',

    'email-address' => 'Correo electrónico',

    'agree-conditions' => 'He leído las <a href="conditions.php">condiciones de uso</a> y estoy de acuerdo con ellas.',

    'ph1' => '4-16 caracteres, sin mayúsculas, el primero debe ser una letra.',

    'ph2' => 'Mín. 6 caracteres',

    'ph3' => 'Ingresa tu correo electrónico',

    'explanation' => 'El usuario de base de datos y el nombre de la base de datos puede contener letras minúsculas, números y guión bajo, y deben tener entre 4 y 16 caracteres de longitud. Usted no debe usar <a href="http://dev.mysql.com/doc/refman/5.7/en/reserved-words.html">palabras reservadas</a>!',

    'maintenance-notice' => 'Debido al mantenimiento de la base de datos, los registros no son posibles actualmente.',

    'agm-p1' => 'Al registrarse para obtener una cuenta db4free.net usted acepta que:',

    'agm-li1' => 'db4free.net es un entorno de prueba',

    'agm-li2' => 'db4free.net no es adecuado para la producción',

    'agm-li3' => 'si usted decide usar su base de datos db4free.net en un entorno de producción a pesar de las advertencias, lo hace bajo su propio riesgo (son muy recomendables copias de seguridad frecuentes)',

    'agm-li4' => 'pérdida de datos e interrupciones pueden ocurrir en cualquier momento (alguna queja acerca de eso es probable que se ignoran)',

    'agm-li5' => 'el equipo de db4free.net no concede ningún tipo de garantía o responsabilidad de cualquier tipo',

    'agm-li6' => 'el equipo de db4free.net se reserva el derecho de eliminar las bases de datos y/o cuentas en cualquier momento sin previo aviso',

    'agm-li7' => 'le corresponde a usted obtener la información más reciente de <a href="/twitter.php">Twitter</a> y del <a href="/blog.php">blog de db4free.net</a>',

    'agm-li8' => 'db4free.net sólo proporciona una base de datos MySQL, pero no espacio en la web (no hay lugar para cargar algún archivo)',

    'agm-p2' => 'Además:',

    'agm-li9' => 'db4free.net es un servicio para pruebas, no para hosting. Las bases de datos que almacenan más de 200 MB de datos se borrarán a intervalos irregulares y sin notificación',

    'agm-li10' => 'Por favor, elimine los datos que ya no necesite, o <a href="/delete-account.php">borre su cuenta si ya no es necesaria</a>. Esto hace que sea más fácil de recuperar si se produce una caída del servidor.',

    'signup-error1' => 'Usted tiene que estar de acuerdo con las condiciones de uso!',

    'signup-error2' => 'Error al registrar los datos de su base de datos MySQL!',

    'signup-error3' => 'Error al enviar el correo electrónico de confirmación!',

    'signup-success' => 'Gracias por registrarse! Usted recibirá un correo electrónico para confirmar el registro.',
);
?>